...
1. Get the cluster name.
   ...
   ```bash
   export AKS_CLUSTER_NAME=$(az deployment group show --resource-group rii-qa -n cluster-stamp --query properties.outputs.aksClusterName.value -o tsv)
   ```
...
1. Get AKS `kubectl` credentials (as a user that has admin permissions to the cluster).
   ...
   ```bash
   az aks get-credentials -g rii-qa -n $AKS_CLUSTER_NAME
   ```
...
1. Import cluster management images to your container registry.
   ...
   export ACR_NAME=$(az deployment group show --resource-group rii-qa -n cluster-stamp --query properties.outputs.containerRegistryName.value -o tsv)
...
1. Deploy Flux.
   ...
   ```bash
   kubectl apply -f https://raw.githubusercontent.com/git-ashiq/aks-fabrikam-dronedelivery/main/cluster-baseline-settings/flux.yaml
   ```
