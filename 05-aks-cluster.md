...
1. Create the AKS cluster and the Azure Container Registry resource groups.
   ...
   az deployment sub create --name workload-stamp-prereqs --location westus2 --template-file ./workload/workload-stamp-prereqs.json --parameters resourceGroupName=rii-qa resourceGroupLocation=westus2

   az deployment sub create --name cluster-stamp-prereqs --location westus2 --template-file cluster-stamp-prereqs.json --parameters resourceGroupName=rii-qa resourceGroupLocation=westus2
   ```
...
1. Get the AKS Fabrikam Drone Delivery 00's user identities
   ...
   ```bash
   WORKFLOW_ID_NAME=$(az deployment group show -g rii-qa -n workload-stamp-prereqs-dep --query properties.outputs.workflowIdName.value -o tsv) && \
   WORKFLOW_ID_PRINCIPAL_ID=$(az identity show -g rii-qa -n $WORKFLOW_ID_NAME --query principalId -o tsv) && \
   INGRESS_CONTROLLER_ID_NAME=$(az deployment group show -g rii-qa -n cluster-stamp-prereqs-identities --query properties.outputs.appGatewayControllerIdName.value -o tsv) && \
   INGRESS_CONTROLLER_ID_PRINCIPAL_ID=$(az identity show -g rii-qa -n $INGRESS_CONTROLLER_ID_NAME --query principalId -o tsv)
   ```
...
1. Get the AKS cluster spoke VNet resource ID.
   ...
   ```bash
   TARGET_VNET_RESOURCE_ID=$(az deployment group show -g rg-qa-enterprise-networking-spokes -n spoke-qa-rii-iot --query properties.outputs.clusterVnetResourceId.value -o tsv)
   ```
...
1. Deploy the Azure Container Registry ARM template.
   ...
   az deployment group create -f ./workload/workload-stamp.json -g rii-qa -p workflowPrincipalId=$WORKFLOW_ID_PRINCIPAL_ID -p acrResourceGroupName=$ACR_RESOURCE_GROUP
   ```

1. Get the Azure Container Registry deployment output variables

   ```bash
   ACR_NAME=$(az deployment group show -g rii-qa -n workload-stamp --query properties.outputs.acrName.value -o tsv) && \
...
1. Deploy the cluster ARM template.
  :exclamation: By default, this deployment will allow unrestricted access to your cluster's API Server. You can limit access to the API Server to a set of well-known IP addresses such as your hub firewall IP, bastion subnet, and build agents. You can do this by providing these IP addresses to the `clusterAuthorizedIPRanges` parameter of the cluster-stamp ARM template.
   ...
   az deployment group create --resource-group rii-qa --template-file cluster-stamp.json --parameters targetVnetResourceId=$TARGET_VNET_RESOURCE_ID k8sRbacAadProfileAdminGroupObjectID=$K8S_RBAC_AAD_PROFILE_ADMIN_GROUP_OBJECTID k8sRbacAadProfileTenantId=$K8S_RBAC_AAD_PROFILE_TENANTID appGatewayListenerCertificate=$APP_GATEWAY_LISTENER_CERTIFICATE aksIngressControllerCertificate=$AKS_INGRESS_CONTROLLER_CERTIFICATE_BASE64 workflowIdName=$WORKFLOW_ID_NAME ingressControllerIdName=$INGRESS_CONTROLLER_ID_NAME ingressControllerPrincipalId=$INGRESS_CONTROLLER_ID_PRINCIPAL_ID acrResourceGroupName=$ACR_RESOURCE_GROUP acrName=$ACR_NAME
   ```
    ...
    **Option 2 - Automated deploy using GitHub Actions (fork is required)**
    ...
    1. Copy the GitHub workflow file into the expected directory and update the placeholders in it.
       ...
       cat github-workflow/aks-deploy.yaml | \
           sed "s#<resource-group-location>#westus2#g" | \
           sed "s#<resource-group-name>#rii-qa#g" | \
           ...
           sed "s#<acr-resource-group-location>#westus2#g" \
