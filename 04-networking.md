...
1. Create a resource group for the hub network.
   ...
   ```bash
   az group create --name rg-qa-enterprise-networking-hubs --location westus2
   ```
...
1. Create a resource group for the spoke network.
   ...
   ```bash
   az group create --name rg-qa-enterprise-networking-spokes --location westus2
   ```

1. Create the regional network hub.

   > :book: When the networking team created the regional hub for westus2, it didn't have any spokes yet defined, yet the networking team always lays out a base hub following a standard pattern (defined in `hub-default.json`). A hub always contains an Azure Firewall (with some org-wide policies), Azure Bastion, a gateway subnet for VPN connectivity, and Azure Monitor for network observability. They follow Microsoft's recommended sizing for the subnets.
   >
   ...
   >
   > In addition to the westus2 regional hub (that you're deploying) you can assume there are similar deployed as well in in other Azure regions in this resource group.

   ```bash
   # [This takes about five minutes to run.]
   az deployment group create --resource-group rg-qa-enterprise-networking-hubs --template-file networking/hub-default.json --parameters location=westus2
   ```
...
1. Create the spoke network into which the AKS cluster and adjacent resources will be deployed.
   ...
   ```bash
   HUB_VNET_ID=$(az deployment group show -g rg-qa-enterprise-networking-hubs -n hub-default --query properties.outputs.hubVnetId.value -o tsv)
   ```
   ...
   az deployment group create --resource-group rg-qa-enterprise-networking-spokes --template-file networking/spoke-qa-rii-iot.json --parameters location=westus2 hubVnetResourceId="${HUB_VNET_ID}"
   ```
   ...
   The spoke creation will emit the following:
     ...
     * `clusterVnetResourceId` - The resource ID of the VNet that the cluster will land in. E.g. `/subscriptions/[subscription id]/resourceGroups/rg-qa-enterprise-networking-hubs/providers/Microsoft.Network/virtualNetworks/vnet-hub-spoke-shipping-dronedelivery`
     * `nodepoolSubnetResourceIds` - An array containing the subnet resource IDs of the AKS node pools in the spoke. E.g. `["/subscriptions/[subscription id]/resourceGroups/rg-qa-enterprise-networking-hubs/providers/Microsoft.Network/virtualNetworks/vnet-hub-spoke-shipping-dronedelivery/subnets/snet-clusternodes"]`

1. Update the shared, regional hub deployment to account for the requirements of the spoke.

   > :book: Now that their hub has its first spoke, the hub can no longer run off of the generic hub template. The networking team creates a named hub template (e.g. `hub-westus2.json`) to forever represent this specific hub and the features this specific hub needs in order to support its spokes' requirements. As new spokes are attached and new requirements arise for the regional hub, they will be added to this template file.

   First, get the resource id of the **snet-clusternode** subnet found in the spoke network. This value is used ..

   ```bash
    NODEPOOL_SUBNET_RESOURCEIDS=$(az deployment group show -g rg-qa-enterprise-networking-spokes -n spoke-qa-rii-iot --query properties.outputs.nodepoolSubnetResourceIds.value -o tsv)
   ```
   ...
   az deployment group create --resource-group rg-qa-enterprise-networking-hubs --template-file networking/hub-westus2.json --parameters location=westus2 nodepoolSubnetResourceIds="['${NODEPOOL_SUBNET_RESOURCEIDS}']" serviceTagsLocation=WestUS2
   ```
